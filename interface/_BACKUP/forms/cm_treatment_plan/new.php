<?php
/*
 * This program creates the cm_treatment_plan
 *
 * @package   OpenEMR
...
if (empty($formid)) {
    $mboquery = sqlquery("SELECT `fctp`.`id` FROM `form_cm_treatment_plan` AS `fctp`
                          INNER JOIN `forms` ON (`fctp`.`id` = `forms`.`form_id`) WHERE
                          `forms`.`deleted` = 0 AND `forms`.`formdir` = 'cm_treatment_plan' AND
                          `forms`.`encounter` = ? ORDER BY `fctp`.`id` DESC", array($encounter));
    if (!empty($mboquery['id'])) {
        $formid = 0 + $mboquery['id'];
    }
}
$obj = $formid ? formFetch("form_cm_treatment_plan", $formid) : array();
?>
<html>
<head>
    ...
    $arrOeUiSettings = array(
        'heading_title' => xl('CM Treatment Plan'),
...
<div id="container_div" class="<?php echo attr($oemr_ui->oeContainer()); ?>">
    ...
    <div class="row">
        <div class="col-sm-12">
            <form method=post <?php echo "name='my_form' " . "action='$rootdir/forms/cm_treatment_plan/save.php?id=" . attr_url($formid) . "'\n"; ?>>
