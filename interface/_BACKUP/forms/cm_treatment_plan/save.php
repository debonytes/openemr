<?php
/*
 * This program saves data from the cm_treatment_plan
 *
 * @package   OpenEMR
...
if (empty($id)) {
    $newid = sqlInsert(
        "INSERT INTO form_cm_treatment_plan SET $sets",
        [
            $pid,
    ...
    addForm($encounter, "CM Treatment Plan", $newid, "cm_treatment_plan", $pid, $userauthorized);
} else {
    sqlStatement(
        "UPDATE form_cm_treatment_plan SET $sets WHERE id = ?",
        [
            $pid,
            $_SESSION["authProvider"],
            $_SESSION["authUser"],
