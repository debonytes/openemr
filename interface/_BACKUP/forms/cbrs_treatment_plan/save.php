<?php
/*
 * This program saves data from the cbrs_treatment_plan
 *
 * @package   OpenEMR
...
if (empty($id)) {
    $newid = sqlInsert(
        "INSERT INTO form_cbrs_treatment_plan SET $sets",
        [
            $pid,
    ...
    addForm($encounter, "CBRS Treatment Plan", $newid, "cbrs_treatment_plan", $pid, $userauthorized);
} else {
    sqlStatement(
        "UPDATE form_cbrs_treatment_plan SET $sets WHERE id = ?",
        [
            $pid,
            $_SESSION["authProvider"],
            $_SESSION["authUser"],
