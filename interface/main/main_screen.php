...
if ($GLOBALS['user_credential_expiration_lockout'] or $GLOBALS['credential_expiration_alert'] >= 0)
{
	...
	if (isICANSRequired($physician_type) and ($icans_alert <= $current_date)) 
	{
		$credentials_expired = true;
		$_SESSION['expiration_msg'] = 1; // only show the expired message once
	}
	elseif (isCarInsRequired($physician_type) and ($car_alert <= $current_date)) 
	{
		$credentials_expired = true;
		$_SESSION['expiration_msg'] = 1; // only show the expired message once

	}
	elseif (isLicenseRequired($physician_type) and ($license_alert <= $current_date))
	{
		$credentials_expired = true;
		$_SESSION['expiration_msg'] = 1; // only show the expired message once
	}
...
// END CREDENTIAL EXPIRATION CHECK


// determine which tabs to open. Take into account that password and credentials can expire simultaneously
if ($is_expired and !$credentials_expired) {
    //display the php file containing the password expiration message.
    $frame1url = "pwd_expires_alert.php?csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken());
    $frame1target = "adm";
} elseif (!$is_expired and $credentials_expired){
    //display the php file containing the password expiration message.
    $frame1url = "expiration_date_alert.php?csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken());
    $frame1target = "msc";
} elseif ($is_expired and $credentials_expired){
	//display the php file containing the password expiration message.
    $frame1url = "pwd_expires_alert.php?csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken());
    $frame1target = "adm";
    $frame2url = "expiration_date_alert.php?csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken());
    $frame2target = "msc";
