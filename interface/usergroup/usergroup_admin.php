...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] =="user_admin") {
    if ($_POST["mode"] == "update") {
      ...
      //Display message when Emergency Login user was activated
		...
		if ($_POST["carDate"]) {
			sqlStatement("update users set carDate = ? where id = ?", array($_POST["carDate"], $_POST["id"]));
		}
		
		if ($_POST["icans"]) {
			sqlStatement("update users set icans = ? where id = ?", array($_POST["icans"], $_POST["id"]));
		}
		
		if ($_POST["licenseDate"]) {
			sqlStatement("update users set licenseDate = ? where id = ?", array($_POST["licenseDate"], $_POST["id"]));
		}
    ...
    if ($_POST["mode"] == "new_user") {
        ...
        if ($doit == true) {
          ...
          //if password expiration option is enabled,  calculate the expiration date of the password
			...
			"', carDate = '"       . add_escape_custom(trim((isset($_POST['carDate']) ? $_POST['carDate'] : ''))) .
			"', icans = '"         . add_escape_custom(trim((isset($_POST['icans']) ? $_POST['icans'] : ''))) .
			"', licenseDate = '"   . add_escape_custom(trim((isset($_POST['licenseDate']) ? $_POST['licenseDate'] : ''))) .
			"'";
