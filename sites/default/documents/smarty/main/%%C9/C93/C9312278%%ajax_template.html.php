...
//         echo "<a href='javascript:newEvt(" . attr_js($startampm) . "," . attr_js($starttimeh) . "," . attr_js($starttimem) . "," . attr_js($Date) . "," . attr_js($providerid) . "," . attr_js($in_cat_id) . ")' title='New Appointment' alt='New Appointment'>";
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        foreach ($events as $event) {
            ...
            $home_phone = $patient['phone_home'];
            $work_phone = $patient['phone_biz'];
      ...
      $cat_arr = [2,3,4,8,11,16,17,18,19,20];
	      $divTitle .= "\n" . $row['name'];
            if (in_array($catid, $cat_arr)) {
                ...
                else if ($catid == 11 || $catid == 16 || $catid == 17 || $catid == 18 || $catid == 19 || $catid == 20) $catname = xl("RESERVED");
                ...
                if ($patientid) {
                    ...
                    $link_title .= xl('Home') . ": " . $home_phone . "\n";
                    $link_title .= xl('Work') . ": " . $work_phone . "\n";
